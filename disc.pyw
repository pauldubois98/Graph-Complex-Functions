...
def disc(f):
    #we ask for the disc properties
    print("Put 'j' for the imaginary part eg: z0=1+2j")
    print("Image of a disc: |z-z0|<R")
    z0=complex(ask('z0=', '0'))
    R=float(ask('R=', 1))
...
I will create N "random" points""")
    ...
    listz=createRandomFromDisc(z0, R, N)
...
class Disc(Frame):
    def __init__(self, boss=None, **arg):
        ...
        #z0
        Label(self, text='Center of the disc (z0)', \
        ...
        #R
        Label(self, text='Radius of the disc (R)', \
              font=('Times', -20, 'bold')).pack()
    ...
    def graph(self):
        if self.updateVals():
            #creating the list of points
            listz=createRandomFromDisc(self.z0, self.R, self.N)
            #plot function
            myPlot(self.f, listz)
